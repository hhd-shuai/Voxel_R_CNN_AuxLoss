...
class VoxelRCNN(Detector3DTemplate):
    ...
    def get_training_loss(self):
        ...
        loss_rpn, tb_dict = self.dense_head.get_loss() #AnchorHeadSingle
        loss = loss + loss_rpn  #tensor(0.1727, device='cuda:0', grad_fn=<AddBackward0>)

        if self.point_head is not None and self.training:
            loss_point, tb_dict = self.point_head.get_loss(tb_dict)
            loss = loss + loss_point
        
        loss_rcnn, tb_dict = self.roi_head.get_loss(tb_dict) #loss_rcnn: tensor(0.1727, device='cuda:0', grad_fn=<AddBackward0>), tb_dict: {'rpn_loss_cls': 0.05177481472492218, 'rpn_loss_loc': 0.11539990454912186, 'rpn_loss_dir': 0.0054987650364637375, 'rpn_loss': 0.17267349362373352, 'rcnn_loss_cls': 0.3683678209781647, 'rcnn_loss_reg': 0.06379708647727966, 'rcnn_loss_corner': 0.03291439637541771, 'rcnn_loss': 0.46507930755615234}
        loss = loss + loss_rcnn #tensor(0.6378, device='cuda:0', grad_fn=<AddBackward0>)
